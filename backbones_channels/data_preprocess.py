...
class ImgDataPreprocessorCh(BaseDataPreprocessor):
    ...
    def forward(self, data: dict, training: bool = False) -> Union[dict, list]:
        ...
        if is_seq_of(_batch_inputs, torch.Tensor):
            ...
            for _batch_input in _batch_inputs:
                ...
                if self._enable_normalize:
                    _batch_input[:3, :, :] = (_batch_input[:3, :, :] - self.mean) / self.std
        ...
        # Process data with `default_collate`.
            ...
            if self._channel_conversion:
                _batch_inputs[:, :3, :, :] = _batch_inputs[:, :3, :, :][:, [2, 1, 0], ...]
            # Convert to float after channel conversion to ensure
            # efficiency
            _batch_inputs = _batch_inputs.float()
            if self._enable_normalize:
                _batch_inputs[:, :3, :, :] = (_batch_inputs[:, :3, :, :] - self.mean) / self.std
            h, w = _batch_inputs.shape[2:4]
            target_h = math.ceil(
...
@MODELS.register_module()
class DetDataPreprocessorCh(ImgDataPreprocessorCh):
